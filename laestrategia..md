...
Si el precio es mayor que 78.6% su operativa será solo compras con un Take Profit o toma de ganancia de cada operación de 0.5$ hasta que sume ganancias diarias entre el 0.5% y 10% (Configurable por el usuario) del capital para luego apagarse y no operar hasta el otro día.
Si el precio es menor que el 23.6% su operativa será solo ventas con un Take Profit o toma de ganancia de cada operación de 0.5$ hasta que sume ganancias diarias entre el 0.5% y 10% (Configurable por el usuario) del capital para luego apagarse y no operar hasta el otro día.
Si el precio está es menor que 0% o mayor que 100% no hace ninguna operación y envía un mensaje de fuera de rango y espera estar dentro de los valores anteriores

Cuando termina de trabajar el bot?
El debe trabajar hasta que se cumplan cualquiera de las 2 condiciones siguientes:
Haber alcanzado el porcentaje diario configurado (Entre el 1 y el 10% diario configurable) y está trabajando de 8 a 11 UTC y a las 9:30 UTC el cumplió el % indicado y tienes compras y ventas abiertas, no va abrir más operaciones, solo va a buscar cerrar las que están, si primero cierra las compras entonces no abre más compras solo le queda esperar que cierren las ventas haciendo su proceso y al cerrarlas entonces no trabaja más hasta el próximo día.
Si el bot está configurado para trabajar de 8 a 11 UTC y son las 11 y no ha logrado cumplir el porcentaje indicado, no va abrir más operaciones, solo va a buscar cerrar las que ya están abiertas, si tiene compras y ventas y cierra todas las compras, no va a abrir más compras, solo va a buscar cerrar las ventas que queden.

Una vez puedas escribir esto debes dejar un espacio en el código con una opción que llamaremos Modo Recovery para evitar los stops loss que es lo que va a marcar la diferencia en nuestro robot, pero vamos primero a que esto quede bien.
